 ...
 * 
 * Usage:
 * 1. Run `node migration-cleanup.js verify` to generate a verification report
 * 2. Review the report to confirm which files should be removed
 * 3. Run `node migration-cleanup.js cleanup` to remove redundant files
 * 
...
function main() {
  ...
  if (!command || !['verify', 'cleanup'].includes(command)) {
    console.log('Usage: node migration-cleanup.js [verify|cleanup]');
    process.exit(1);
  }
